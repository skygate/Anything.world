...
// const ConnectButton = styled(WalletDialogButton)``; // <- connect button styles override
...
const Home = (props: HomeProps) => {
  ...
  const handleClick = useCallback(
    (event: Event) => {
      if (connectWallet) connectWallet(event);
      if (!event.defaultPrevented) setOpen(true);
    },
    [connectWallet, setOpen]
  );

  useEffect(() => {
    const mintButton = document.getElementsByClassName("minting")[0];

    mintButton.addEventListener("click", handleClick);

    return () => mintButton.removeEventListener("click", handleClick);
  });

  useEffect(() => {
    if (connected) {
      const mintButton = document.getElementsByClassName("minting")[0];
      mintButton.removeEventListener("click", handleClick);
    }
  }, [connected]);
